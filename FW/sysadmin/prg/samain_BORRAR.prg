...
*	Clientes\Pyme\sync_pyme.prg

External Procedure ;
	Fw\SysAdmin\mnx\sistema.prg,;
	GetUpdatableFieldList.prg

External File FenixLogo.JPG
...
*!*		Do GetEntity.prg
*!*		Do LogError.prg
*!*		Do v:\Clipper2Fox\Tools\eMail\Prg\prxSmtp.prg
*!*	EndIf
...
Define Class prxApplication As AbstractApplication Of "FW\TierAdapter\Comun\AbstractApplication.prg"
	...
	* ( Ver NewColDataBases.prg )
	cDataDictionaryLib 		= "Clientes\Pyme\sync_Pyme.prg"
	cDataDictionaryClass 	= "Pymes"
	...
	cScreenIcon 			= "v:\Clipper2Fox\fw\Comunes\Image\ico\Fenix.ico"
	...
	*!*
	...
	Procedure SetApplicationMainCaption( a, b ) As Void
		...
		Try
			...
			This.oError.cTraceLogin = "Estableciendo Títulos Principales"

			lcCaption = Strtran( Alltrim( This.cApplicationName ), "_", " " )
	...
	*!*
	...
	Procedure Login( tlForce As Boolean ) As Boolean;
			HELPSTRING "Loguea un usuario al sistema"

		Local lnClave As Integer
		Local loUser As User Of "Fw\TierAdapter\Comun\prxUser.prg"
		Local loScreenLog As cntScreenLog Of v:\clipper2fox\Rutinas\vcx\clipper2fox.vcx
	...
	Procedure ValidateModulo( oUser As Object )
		...
		Try
			...
			If IsRuntime()
				...
				*!*					If  FileExist( lcRootWorkFolder + "Modulos.Dbf" )

				*!*						TEXT To lcCommand NoShow TextMerge Pretext 15
				*!*							Select * From '<<lcRootWorkFolder>>Modulos.Dbf' Into Cursor cModulos
				*!*						ENDTEXT
			...
			If  FileExist( Addbs( DRCOMUN ) + "Modulos.Dbf" )

				TEXT To lcCommand NoShow TextMerge Pretext 15
				Select * From '<<Addbs( DRCOMUN )>>Modulos.Dbf' Into Cursor cModulos
				ENDTEXT
			...
			If Used( "cModulos" )
				...
				If Upper( Field( "ID", "cModulos" ) ) = "ID"
					Locate For Id = This.nModuloId

				Else
					Locate For Alltrim( Upper( cModulos.Nombre )) == Alltrim( Upper( lcAplicacion ))
				...
				If Found()
					...
					If Upper( Field( "NIVEL", "cModulos" ) ) = "NIVEL"
						lnNivel = cModulos.Nivel

					Else
						llOk = .F.

						If Empty( Field( "Activo", "cModulos" ))
							llActivo = .T.

						Else
							llActivo = cModulos.ACTIVO
	...
	Procedure oUser_Access(  ) As Object;
		...
		Try
			...
			If Vartype(This.oUser)#"O"
				*!*					This.oUser = CreateObjParam( "Descripcion", "",;
				*!*						"Nombre", "",;
				*!*						"IsAdmin", .F.,;
				*!*						"IsGuest", .T.,;
				*!*						"nEmpresaActivaId", 0,;
	...
	* Setea las variables públicas
	Procedure SetGlobales(  ) As Void;
		...
		Try
			...
			If !Empty( This.cParameFileName )
				loRestoreVars = Createobject( "oRestoreVars" )
				loRestoreVars.RestoreFrom( This.cParameFileName )
			Endif
...
Define Class oRestoreVars As Session
	...
	#If .F.
		Local This As oRestoreVars Of "V:\Clipper2fox\Fw\Sysadmin\Prg\Samain.prg"
	#Endif
