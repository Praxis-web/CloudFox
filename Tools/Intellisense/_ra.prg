...
TEXT To lcCode NoShow
    ...
    Set Century On
    *lcRet = '* RA ' + Transform( Year( Date() ) ) + '-' + Transform( Month( Date() ) , '@L 99' ) + '-' +  Transform( Day( Date() ) , '@L 99') + '(' + Time() + ')'
    lcRet = '* RA ' + Dtoc( Date()) + '(' + Time() + ')'
...
Delete From UdpFoxCode Where Type = 'U' And ABBREV = '_ra'
insert into UdpFoxCode ( Type, ABBREV, cmd, Data )  Values ( 'U', '_ra', '{}', lcCode )
Use In Select( 'UdpFoxCode' )