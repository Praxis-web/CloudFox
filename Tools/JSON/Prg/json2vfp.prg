...
Local loJSON As prxJSON Of "Tools\Json\Prg\Json2Vfp.prg"
...
Try
    ...
    *Set Step On
    ...
    loJSON = Newobject( "prxJSON", "Tools\JSON\Prg\JSON2Vfp.prg" )
    loJSON.lDebug = .T.

    TEXT To lcCommand NoShow TextMerge Pretext 15
    Create Cursor cCabeza (
    	id I,
    	Nombre C(30),
        activo L,
        orden I,
    ...
    Select cCabeza
    loReg = ScatterReg( .T., "cCabeza" )
    loReg.Id = i
    loReg.nombre = "CABEZA " + StrZero( 1, 7 )
    loReg.activo = .T.
    loReg.orden = 7
    loReg.codigo = StrZero( 1, 7 )
    loReg.cliente_praxis = 1
    loReg.empresa = 1
    Append Blank
    Gather Name loReg
    
    loFactura = CreateObject( "Empty" )
    AddProperty( loFactura, "Cabeza", loReg )

	Select cDetalle
    loMainCol = Createobject( "Collection" )
    For i = 1 To 1000

        loReg = ScatterReg( .T., "cGrupos" )
        loReg.Id = i
        loReg.nombre = "DETALLE " + StrZero( i, 7 )
        loReg.activo = .T.
        loReg.orden = 7
        loReg.codigo = StrZero( i, 7 )
        loReg.cliente_praxis = 1
        loReg.empresa = 1
        Append Blank
        Gather Name loReg
    ...
    AddProperty( loFactura, "Detalle", loMainCol )

    Browse

    Select cPie
    loReg = ScatterReg( .T., "cPie" )
    loReg.Id = i
    loReg.nombre = "PIE " + StrZero( 1, 7 )
    loReg.activo = .T.
    loReg.orden = 7
    loReg.codigo = StrZero( 1, 7 )
    loReg.cliente_praxis = 1
    ...
    AddProperty( loFactura, "Pie", loReg )

    lcJSON_Data = loJSON.VfpToJson( loFactura )

    Inform( lcJSON_Data )
    Strtofile( lcJSON_Data, "Test.json" )
...
Define Class prxJSON As Custom
    ...
    #If .F.
        Local This As prxJSON Of "Tools\Json2Vfp\Prg\Json2Vfp.prg"
    #Endif
    ...
    *
    * Recibe un objeto VFP y lo devuelve  en Json2Vfp
    Procedure VfpToJson( oObj As Object ) As String ;
            HELPSTRING "Recibe un objeto VFP y lo devuelve  en Json2Vfp"
        Local lcCommand As String,;
            lcJSON_Str As String,;
            lcName As String,;
            lcExact As String,;
